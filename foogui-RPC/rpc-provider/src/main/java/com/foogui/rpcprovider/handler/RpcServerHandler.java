...
@Slf4j
@Component
@ChannelHandler.Sharable     //表示该ChannelHandler实现类可以被共享使用，即可以被多个Channel共用，而不必每次都创建一个新的实例。这样可以节省资源，提高性能
public class RpcServerHandler extends SimpleChannelInboundHandler<String> implements ApplicationContextAware {

    private static final Map<String, Object> SERVICE_INSTANCE_MAP = new ConcurrentHashMap<>();

    // 在Netty应用程序中，通常会使用ChannelHandlerContext来与ChannelPipeline进行交互，而使用SimpleChannelInboundHandler来处理入站消息。
    @Override
    ...
    /**
     * 将服务端所有标记了@RpcService的Bean放入缓存存起来
     *
